...
func (s *Store) GetBlockedSigns() ([]*models.BlockedSign, error) {
	rows, err := s.db.Query("SELECT id, latitude, longitude FROM blocked_signs")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CreateStationPoint(sp *models.StationPoint) error {
	query := "INSERT INTO station_points (latitude, longitude) VALUES ($1, $2) RETURNING id"
	return s.db.QueryRow(query, sp.Latitude, sp.Longitude).Scan(&sp.ID)
}

// GetStationPoints retrieves all station points from the database.
func (s *Store) GetStationPoints() ([]*models.StationPoint, error) {
	rows, err := s.db.Query("SELECT id, latitude, longitude FROM station_points")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
